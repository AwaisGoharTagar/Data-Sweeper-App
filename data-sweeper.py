...
# st.set_page_config(page_title="Data Sweeper", layout="wide")
# st.title("Data Sweeper")
# st.write("Convert and clean your files with built-in processing and visualization!")
...
# st.set_page_config(page_title="Data Sweeper", layout="wide")
# st.title("Data Sweeper")
# st.write("Transform and clean your files (CSV, Excel, Word, PowerPoint, PDF) with built-in data processing and visualization!")
...
#             doc = docx.Document(file)
#             text = "\n".join([para.text for para in doc.paragraphs])
#             st.text_area(f"Content of {file.name}", text, height=200)
#             continue
#         elif file_ext == ".pdf":
#             pdf_reader = PdfReader(file)
#             text = "\n".join([page.extract_text() for page in pdf_reader.pages if page.extract_text()])
...
#             presentation = pptx.Presentation(file)
#             slides_text = "\n".join([shape.text for slide in presentation.slides for shape in slide.shapes if hasattr(shape, "text")])
#             st.text_area(f"Content of {file.name}", slides_text, height=200)
#             continue
#         else:
#             st.error(f"Unsupported file type: {file_ext}")
#             continue
...
#                 for _, row in df.iterrows():
#                     pdf.multi_cell(190, 10, txt=", ".join(map(str, row.values)))
